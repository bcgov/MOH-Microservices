...
export const generateServiceCommand = (override) => {
  ...
  return `PORT=${options.PORT} HOSTNAME=${options.HOSTNAME} TARGET_URL=${options.TARGET_URL} AUTH_TOKEN_KEY=${options.AUTH_TOKEN_KEY} USE_AUTH_TOKEN=${options.USE_AUTH_TOKEN} LOGGER_HOST=${options.LOGGER_HOST} LOGGER_PORT=${options.LOGGER_PORT} NOUN_JSON='${options.NOUN_JSON}' RATE_LIMIT=${options.RATE_LIMIT} timeout ${options.timeout} node src/index.js server`;
