...
describe("Start local servers, test APIs", async () => {
  ...
  it("(Spa env server) Should respond with a 200 to the /env endpoint when SPA_ENV_NAME header is present and formatted correctly (singleton)", async () => {
  ...
  it("(Spa env server) Should respond with a 200 to the /env endpoint when SPA_ENV_NAME header is present and formatted correctly (JSON)", async () => {
    let headers = new Headers();
    headers.set("Authorization", "spaenv XXX");
    headers.set("SPA_ENV_NAME", "SPA_ENV_NOW");
    const response = await fetch(`${localServiceUrl}/env`, {
